...
class InterpolateUpsampling(AbstractUpsampling):
    ...
    #def __init__(self, mode='nearest', scale_factor=(1,1,1)):
    def __init__(self, mode='nearest'):
        #upsample = partial(self._interpolate, mode=mode,scale_factor=scale_factor)
        upsample = partial(self._interpolate, mode=mode)
        super().__init__(upsample)

    @staticmethod
    def _interpolate(x, size, mode):
        ret = F.interpolate(x, size=size, mode=mode) #torch.nn.functional.interpolate
        return ret
